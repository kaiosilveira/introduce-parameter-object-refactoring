...
Oftentimes we see a group of parameters being used repeatedly as arguments for multiple functions. These groups are often suggesting a hidden structure inside the project's domain. When this pattern is detected, we can use _Introduce Parameter Object_ to create a class based on these parameters and to use it instead. This refactoring helps with this process.

## Working example

As our working example for this refactoring, we have the `readingsOutsideRange` function, responsible for finding the readings that falls outside of a specified range. This function has the `max` and `min` parameters, that are actually hiding a bigger, `NumberRange` structure that can be created to aid in this comparison.

### Before

The core initial code for the refactoring is:
...
The final code for this refactoring looks like:
...
A simple test suite with one test was put in place to make sure the `readingsOutsideRange` function is behaving as expected. Corner cases and validations were left out for simplicity. The test suite looks like:
...
You can also see the full commit history at the [Commit History tab](https://github.com/kaiosilveira/introduce-parameter-object-refactoring/commits/main).
